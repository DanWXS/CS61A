...
class TutorCard(Card):
    ...
    def power(self, opponent_card):
        return opponent_card.power(self) - float('inf')
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if len(player.hand):
            max_power = player.hand[0].power(opponent_card)
            for curr_card in player.hand:
                if curr_card.power(opponent_card) >= max_power:
                    max_power = curr_card.power(opponent_card)
                    best_card = curr_card
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        re_add = True
        if self.attack and self.defense >=0:
            re_add = True
