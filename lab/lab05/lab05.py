...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    dist_a = sqrt((lat-get_lat(city_a))**2+(lon-get_lon(city_a))**2)
    dist_b = sqrt((lat-get_lat(city_b))**2+(lon-get_lon(city_b))**2)
    if dist_a < dist_b:
...
def coords(fn, seq, lower, upper):
    ...
    [[-2, 4], [1, 1], [3, 9]]
    """
    return [[item,fn(item)] for item in seq if (fn(item) in range(lower,upper+1))]
