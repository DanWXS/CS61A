...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if ((not place.ant.can_contain(self) and not self.can_contain(place.ant))
                    or (not place.ant.is_container and not self.is_container)):
                assert place.ant is None, 'Too many ants in {0}'.format(place)
            else:
                if place.ant.is_container:
                    place.ant.store_ant(self)
                else:
                    contained_ant = place.ant
                    place.ant = self
                    self.store_ant(contained_ant)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            if current_place.bees and self.lower_bound <= count <= self.upper_bound:
                attack_place = current_place
                break
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            damage += self.damage
        for one_bee in bees_lst:
            one_bee.reduce_health(damage)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if other.is_container :
            self.can_contain_status = False
        return self.can_contain_status
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        if self.place.bees:
            for one_bee in bees:
                one_bee.reduce_health(self.damage)
    ...
    def action(self, gamestate):
        ...
        while curr_place:
            if curr_place.ant:
                ...
                if curr_place.ant.is_container and curr_place.ant.ant_contained:
