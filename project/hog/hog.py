...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls:
        ...
        if curr ==1:
            sow_sad = True
        num_rolls-=1
    return 1 if sow_sad == True else sum
    # END PROBLEM 1
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average_dice(*args):
        ...
        for i in range(total_samples):
           ...
           i+=1
        return float(total/total_samples)
    return average_dice
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max = average_dice(10,dice)
    i=10
    num = 10
    while i:
        curr_average = average_dice(i,dice)
        if curr_average >=max:
            max = curr_average
            num = i
        i-=1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    gap = square_update(0,score,opponent_score)-score
    if gap>=threshold:
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    for i in (1,2):
        if roll_dice(i)+score>=GOAL:
            return i
    if score>opponent_score:
        return 3
    average_dice = make_averaged(roll_dice)
    if tail_points(opponent_score)>average_dice(6) or score+tail_points(opponent_score)>=GOAL:
        return 0
    else:
